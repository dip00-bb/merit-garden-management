...
class AdmissionInformation(ctk.CTkFrame):
    ...
    def on_to_admit_class_change(self,selected_class):
        if ( int(selected_class) > 8):
            ...
            self.student_group_option.configure(state="disabled")
            self.student_optional_subject_option.configure(state="disabled")
            self.student_group_option.set("Group")
            self.student_optional_subject_option.set("Optional Subject")
            
    def on_group_change(self,selected_group):
        if (selected_group=="arts" or selected_group=="commerce" ):
            self.student_optional_subject_option.configure(values=[self.optional_subject[1]],state="enabled")
        else:
            self.student_optional_subject_option.configure(values=self.optional_subject,state="enabled")